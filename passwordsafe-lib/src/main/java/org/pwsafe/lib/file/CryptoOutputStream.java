...
/**
 * This class is used to encrypt an existing OutputStream while providing an
 * OutputStream interface itself.
 * 
 * Note that because of the block nature of encryption there will normally be
 * extra bytes at the end of such a stream to represented the encrypted
 * "zero padding" added at the end of the original stream.
 * 
 * Also because of the block nature of this stream, there is implicit (and
 * unflushable) buffering involved so closing the stream is extremely important.
 * 
 * @author mtiller
 * 
 */
public class CryptoOutputStream extends OutputStream {
	...
	/**
	 * a routine to initialize the encryption data structures and generate some
	 * random header data.
	 * 
	 * @throws IOException
	 */
	private void initialize() throws IOException {
		...
		final byte[] temp = Util.cloneByteArray(randStuff, 10);
