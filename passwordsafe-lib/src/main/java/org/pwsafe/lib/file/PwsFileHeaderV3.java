...
public class PwsFileHeaderV3 implements Serializable {
	 ...
	 *         the data.
	 */
	public PwsFileHeaderV3(PwsFile file) throws IOException, EndOfFileException {
		file.readBytes(tag);
		file.readBytes(salt);
	 ...
	 *         the data.
	 */
	public PwsFileHeaderV3(InputStream stream) throws IOException, EndOfFileException {
		stream.read(tag);
		stream.read(salt);
		final byte[] iterBytes = new byte[4];
	 ...
	 * @throws IOException
	 */
	public void save(PwsFile file) throws IOException {
