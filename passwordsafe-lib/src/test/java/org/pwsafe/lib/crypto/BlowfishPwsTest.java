...
public class BlowfishPwsTest extends TestCase {
	...
	int[] key = { 0x9F, 0x58, 0x9F, 0x5C, 0xF6, 0x12, 0x2C, 0x32, 0xB6, 0xBF, 0xEC, 0x2F, 0x2A,
			0xE8, 0xC3, 0x5A };
	int[] plainText = { 0xD4, 0x91, 0xDB, 0x16, 0xE7, 0xB1, 0xC3, 0x9E, 0x86, 0xCB, 0x08, 0x6B,
			0x78, 0x9F, 0x54, 0x19 };
	int[] cipherText = { 0xA4, 0x3D, 0x6E, 0x0B, 0x0F, 0xD6, 0xEF, 0xAA, 0xBF, 0x74, 0xDE, 0x87,
			0x38, 0x53, 0x91, 0x5E };

	int[] key32 = { 0xD4, 0x3B, 0xB7, 0x55, 0x6E, 0xA3, 0x2E, 0x46, 0xF2, 0xA2, 0x82, 0xB7, 0xD4,
			0x5B, 0x4E, 0x0D, 0x57, 0xFF, 0x73, 0x9D, 0x4D, 0xC9, 0x2C, 0x1B, 0xD7, 0xFC, 0x01,
			0x70, 0x0C, 0xC8, 0x21, 0x6F };
	int[] plainText32 = { 0x90, 0xAF, 0xE9, 0x1B, 0xB2, 0x88, 0x54, 0x4F, 0x2C, 0x32, 0xDC, 0x23,
			0x9B, 0x26, 0x35, 0xE6 };
	int[] cipherText32 = { 0x4F, 0x16, 0xBC, 0x11, 0x57, 0x4A, 0x9D, 0x55, 0xA1, 0xA2, 0x33, 0xAA,
			0xA8, 0x05, 0xD2, 0x5C };
	...
	public void BCDecrypt() throws PasswordSafeException {
		...
		final BlowfishPws bc16 = new BlowfishPws(k16);
		final BlowfishPws bc32 = new BlowfishPws(k32);

		final byte[] buf16 = Util.unsignedToSigned(cipherText);
		final byte[] buf32 = Util.unsignedToSigned(cipherText32);
	...
	public void runBCRoundTrip() throws PasswordSafeException {
		...
		final BlowfishPws ebc16 = new BlowfishPws(k16, iv, true);
		final BlowfishPws ebc32 = new BlowfishPws(k32, iv, true);
		final BlowfishPws ej16 = new BlowfishPws(k16, iv);
		final BlowfishPws ej32 = new BlowfishPws(k32, iv);
		final BlowfishPws dbc16 = new BlowfishPws(k16, iv, true);
		final BlowfishPws dbc32 = new BlowfishPws(k32, iv, true);

		final byte[] buf16 = new byte[64];
		Util.newRandBytes(buf16);
		byte[] orig = Util.cloneByteArray(buf16);
		final byte[] j16 = Util.cloneByteArray(buf16);
		...
		final byte[] buf32 = new byte[64];
		Util.newRandBytes(buf32);
		orig = Util.cloneByteArray(buf32);
		final byte[] j32 = Util.cloneByteArray(buf32);
	...
	public void runBareBC(byte[] key, byte[] orig, byte[] expct) {
		...
		final KeyParameter ekp = new KeyParameter(Util.cloneByteArray(key));
		final ParametersWithIV eiv = new ParametersWithIV(ekp, iv);

		cipher.init(true, eiv);

		Util.bytesToLittleEndian(orig);
		byte[] buf16 = Util.cloneByteArray(orig);
		final byte[] enc = new byte[buf16.length];
		...
		Util.bytesToLittleEndian(enc);
		final KeyParameter dkp = new KeyParameter(Util.cloneByteArray(key));
		final ParametersWithIV div = new ParametersWithIV(dkp, iv);

		buf16 = Util.cloneByteArray(enc);
		cipher = new CBCBlockCipher(new BlowfishEngine());
		cipher.init(false, div);

		final byte[] dec = new byte[buf16.length];
	...
	public void testBaselineECB() throws Exception {
		...
		final byte[] testKey1 = { (byte) 0x1c, (byte) 0x58, (byte) 0x7f, (byte) 0x1c, (byte) 0x13,
				(byte) 0x92, (byte) 0x4f, (byte) 0xef };
		final byte[] invTestKey1 = new byte[testKey1.length];
		Util.bytesToLittleEndian(invTestKey1);
		final int[] tv_p1 = { 0x30553228, 0x6d6f295a };
		final byte[] tv_p1b = convert(tv_p1);
		final int[] tv_c1 = { 0x55cb3774, 0xd13ef201 };
		final byte[] tv_c1b = convert(tv_c1);
		...
		final int[] tv_p2 = { 0xfedcba98, 0x76543210 };
		final byte[] tv_p2b = convert(tv_p2);
		final int[] tv_c2 = { 0xcc91732b, 0x8022f684 };
		final byte[] tv_c2b = convert(tv_c2);
		byte[] tv_t2b = new byte[8];

		// start the tests, check for a proper decryption, too
		final BlowfishPwsECB testpws1 = new BlowfishPwsECB(testKey1);
		...
		final BlowfishPwsECB testpws2 = new BlowfishPwsECB(testKey2);
	...
	public byte[] convert(int[] v) {
		final byte[] b = new byte[v.length * 4];
