...
public class TestUtils {
	...
	public static void addDummyRecords(PwsFileV3 file, int amount) throws PasswordSafeException {
		for (int i = 0; i < amount; i++) {
			...
			final PwsRecordV3 record = (PwsRecordV3) file.newRecord();
			...
			record.setField(new PwsStringUnicodeField(PwsRecordV3.URL,
					"http://www.nowhere.all/file" + i));
			file.add(record);
		}
	}
	...
	public static void addDummyRecords(PwsFileV2 file, int amount) throws PasswordSafeException {
		for (int i = 0; i < amount; i++) {
			...
			final PwsRecordV2 record = (PwsRecordV2) file.newRecord();
	...
	public static PwsFileV3 createPwsFileV3(String filename, StringBuilder password)
			throws IOException {
		final PwsFileV3 pwsFileV3 = new PwsFileV3();
