...
/**
 * Open command that opens a new safe with the opposite read-write mode than the
 * one opened at the moment. The label and tooltip is set according to the
...
public class OpenFileReadWriteToggleAction extends Action {
	...
	public OpenFileReadWriteToggleAction() {
		...
		setImageDescriptor(ImageDescriptor.createFromURL(this.getClass().getClassLoader()
				.getResource("org/pwsafe/passwordsafeswt/images/tool_newbar_open.gif"))); //$NON-NLS-1$
